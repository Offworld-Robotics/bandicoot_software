...
Rotator::Rotator(float len, float thicc, float startRot) : Segment(len, thicc, startRot) {}

Rotator::Rotator(float len, float thicc, float startRot, float min, float max) : Segment(len, thicc, startRot, min, max) {}

void Rotator::calculateEndDirection(std::vector<float>* basis) {
    ...
    // rotate the 'rotation' vector around 'direction' vector axis by
    ...
    */
    ...
    float w_0 = direction[0];
    float w_1 = direction[1];
    float w_2 = direction[2];

    result.push_back(rotation[0] * (cos_ + w_0 * w_0 * cos_min)        + rotation[1] * (w_0 * w_1 * cos_min - w_2 * sin_) + rotation[2] * (w_1 * sin_ + w_0 * w_2 * cos_min));
    result.push_back(rotation[0] * (w_2 * sin_ + w_0 * w_1 * cos_min)  + rotation[1] * (cos_ + w_1 * w_1 * cos_min)       + rotation[2] * (-w_0 * sin_ + w_1 * w_2 * cos_min));
    result.push_back(rotation[0] * (-w_1 * sin_ + w_0 * w_2 * cos_min) + rotation[1] * (w_0 * sin_ + w_1 * w_2 * cos_min) + rotation[2] * (cos_ + w_2 * w_2 * cos_min));

    std::cout << "Res: " << result[0] << " " << result[1] << " " << result[2] << std::endl;

    // Might not need explicit return TODO check this
    basis[0] = direction;
    basis[1] = result;
    ...
    if (fabs(dot - 0) < FLT_EPSILON) {
        std::cout << "rotator output good" << std::endl;
    } else {
        std::cout << "rotator output bad " << dot << std::endl;
    }
}

void Rotator::generateCollision(std::vector<float> start, std::vector<float> end, float precision) {
